...
independent of UnifiedTrader_EA's own Telegram side - a bad bot token or a
Telegram outage here can never affect trading, and vice versa.
...
both "" until you set them (first-run questions / --setup), and send_alert() is a safe
...
def format_full_conviction_message(symbol: str, verdict, executed: bool,
    ...
    if plan is not None:
        ...
        if getattr(plan, "leg_tp", 0.0):
            # Split entry: leg 1 takes profit at its own TP1; the rest go to
            # break-even at the lock level (tp1_price) and trail.
            ...
            if len(leg_lots) > 1:
                ...
                lines.append(f"Other {len(leg_lots) - 1} leg(s) ({rest:.2f} lot): stop to break-even "
                             f"at {f(plan.tp1_price)}, then trails {f(plan.trail_distance)} behind price")
...
def format_heartbeat_message(symbol: str, minutes_since_last_success: float,
    ...
    return (f"Heartbeat: GoldTrader ({symbol}) is running - last successful evaluation "
...
def format_stale_cycle_alert(symbol: str, minutes_since_last_success: float) -> str:
    return (f"WARNING: GoldTrader ({symbol}) has had no successful evaluation cycle in "
