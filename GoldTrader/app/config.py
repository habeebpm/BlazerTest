...
class AdvisorConfig:
    ...
    # tagged in the order comment - see legs.py). A lot too small to split
    # three ways uses fewer legs (0.02 -> 2, 0.01 -> 1, the take-profit leg).
    # False = one position with the $6 lock and $3 trail. Gold only - the
    ...
    # UnifiedTrader_EA.mq5's InpReferenceLot,
    ...
    # halts - see docs/REFERENCE.md's "Risk parameters". executor.execute()'s open-risk
    ...
    # --- Daily loss circuit breaker - main.py's
    ...
    #     fixed dollar amounts either way: UnifiedTrader_EA.mq5's exit
    ...
    # this system's own `magic`). Set this when UnifiedTrader_EA is also opening same-direction XAUUSD positions
    ...
    # way). Completely independent of UnifiedTrader_EA's own Telegram side. Reusing UnifiedTrader_EA's bot with your own chat
    ...
    # --- Logging ---
    log_dir: str = paths.LOG_DIR     # GoldTrader/logs - see paths.py
