"""
Runs the Telegram relay bridge (relay/telegram_relay_bridge.py) as a
...
Behavior:
  - started with --no-login: it never waits for a phone code in the
    background (log in once with relay_login.bat);
...
BRIDGE_DIR = paths.RELAY_DIR
BRIDGE_SCRIPT = "telegram_relay_bridge.py"
def child_env() -> dict:
    """Environment for every companion program: UTF-8 output always (Windows
    would otherwise write redirected output as cp1252 and fail on the first
    emoji in a relayed Telegram message or in Claude's reasoning)."""
    env = dict(os.environ)
    env["PYTHONUTF8"] = "1"
...
def login(bridge_dir: str = BRIDGE_DIR) -> int:
    ...
    return subprocess.call([sys.executable, BRIDGE_SCRIPT, "--check"], cwd=bridge_dir, env=child_env())
