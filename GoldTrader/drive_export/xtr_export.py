...
on every new M1 candle close (--trigger-timeframe), optionally pushing each
file straight to a Google Drive folder. See docs/REFERENCE.md for the full spec this
...
def last_closed_bar_time(symbol: str, timeframe: str = DEFAULT_TRIGGER_TIMEFRAME):
...
def main(argv: list | None = None) -> int:
    ...
    parser.add_argument("--trigger-timeframe", default=DEFAULT_TRIGGER_TIMEFRAME,
                        dest="trigger_timeframe",
                        help=f"export whenever a bar of this timeframe closes (default "
                             f"{DEFAULT_TRIGGER_TIMEFRAME}; M5 restores the old once-per-5-minutes pace)")
    parser.add_argument("--poll-seconds", type=int, default=5, dest="poll_seconds",
                        help="how often to check for a newly closed trigger bar (default 5)")
    ...
    parser.add_argument("--upload-drive", action="store_true", dest="upload_drive",
                        help="also push every export to Google Drive (needs --drive-folder-id "
                             "and --drive-credentials) - see docs/REFERENCE.md if your MT5 machine has "
    ...
    trigger_tf = args.trigger_timeframe.strip().upper()
    if trigger_tf not in gw.TIMEFRAMES:
        parser.error(f"--trigger-timeframe must be one of {list(gw.TIMEFRAMES)}")
    ...
    log.info("Watching %s %s for a new closed candle every %ds - Ctrl+C to stop.",
              args.symbol, trigger_tf, args.poll_seconds)
    # A distinct sentinel, not None - last_closed_bar_time() itself legitimately
    # returns None when fewer than 2 trigger bars exist yet (e.g. right after
