...
| Claude exits | $6 SL; every entry is three legs with the same stop - leg 1 closes at +$4 (take-profit), legs 2-3 go to break-even at +$6 and trail $3 (the stop is at +$3 the moment +$6 is reached). All $ at the 0.01 reference lot. `--no-claude-split` = one position locked at +$6, then trailed $3 | `app/config.py` `claude_split` (main.py sends the legs; the EA manages them) |
...
| Positions | max 5 trades per direction, both sources together - the legs of a split entry count as one trade | EA + `app/config.py` |
...
| Claude entries as three legs: +$4 take-profit / two legs break-even at +$6 + $3 trail | `start.bat` log on an entry: `ACCEPTED BUY ... 3 leg(s) ... - leg 1 tp=..., the rest break-even at +$6 then $3 trail`; MT5 shows three positions (comments `Claude_Sig`, `Claude_Sig|L2`, `Claude_Sig|L3`, only the first with a TP); Experts tab at start: "Claude split entries (sent by main.py) ..."; journal column `leg` |
