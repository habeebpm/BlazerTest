...
| XTR HTF filter | `InpXtrHtfFilter` (on): a Telegram entry is skipped when M15 or H1 is **clearly** against it (EMA9 vs EMA21, RSI14 vs 50 and MACD histogram all agreeing on its last closed bar) - the same rule ClaudeSMC_Trader's `xtr_logic.py` applies | - |
