...
switched on in main_preset.ini (next to main.py), so one command runs
everything:

    [relay_bridge]        ../../python/telegram_relay_bridge.py   continuous
    [xtr_export]          ../../XTR_Export/python/xtr_export.py    continuous
...
Deliberately NOT included: ../../python/telegram_copier.py - UnifiedTrader_EA
already copies the Telegram signals; running both would double every trade.
"""
...
DEFAULT_PRESET = os.path.join(HERE, "main_preset.ini")
XTR_EXPORT_DIR = os.path.normpath(os.path.join(HERE, "..", "..", "XTR_Export", "python"))

XTR_EXPORT_FATAL = {2: "bad options in main_preset.ini [xtr_export] args (see the lines above)"}
        ...
        try:
            ...
            with open(self.log_path, "a", encoding="utf-8") as out:
                ...
                rc = subprocess.run(self.command, cwd=self.cwd, stdin=subprocess.DEVNULL, stdout=out,
                                    stderr=subprocess.STDOUT, timeout=self.timeout).returncode
...
def start_services(cfg, preset: Preset, alert=None, force_relay: bool = False,
    ...
    for err in preset.errors:
        log.error("main_preset.ini: %s", err)
    started = []
    py = sys.executable
    ...
    log.info("Companion programs: %s", ", ".join(names) if names else "none (see main_preset.ini)")
    return started
