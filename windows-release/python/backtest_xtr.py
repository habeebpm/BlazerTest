...
def selftest() -> None:
    ...
    cfg = bot.BotConfig(
        ...
        require_full_htf_conviction=False, enable_cot=False,   # selftest is offline-only
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--fixed-lot", type=float, default=0.1,
                         help="use this lot size for every trade instead of risk-based sizing "
                              "(default 0.1, always applied, mirroring the live bot's default); "
                              "pass 0 to switch to the system's recommended (risk-based) lot size instead")
    ...
    cfg = bot.BotConfig(
        data_file=Path("unused"), signal_file=Path("unused"), ack_file=Path("unused"),
        outcome_file=Path("unused"), state_file=Path("unused"),
        model=args.model, api_key=api_key, risk_percent=args.risk_percent, fixed_lot=args.fixed_lot,
